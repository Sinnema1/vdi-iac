...
BeforeAll {
    ...
    function NewBuildEvidence {
        ...
        if ($null -eq $Phases) {
            $Phases = @(
                [ordered]@{ name = 'media-qualification'; outcome = 'passed'; reasonCode = $null }
...
Describe 'artifact identity appears exactly when a build sealed' {
    ...
    It 'accepts a sealed result carrying an artifact identity' {
        ...
        Test-SealedCandidate -Evidence $evidence | Should -BeTrue
    }
    ...
    It 'refuses a sealed result with no artifact identity' {
        ...
        $evidence = AsObject (NewBuildEvidence)
        Test-ImageBuildResult -Evidence $evidence | Should -Be 'sealed_without_artifact_identity'
        Test-SealedCandidate -Evidence $evidence | Should -BeFalse
    }
    ...
    It 'refuses a <state> result carrying an artifact identity' -ForEach @(
        ...
        $evidence = AsObject (NewBuildEvidence -BuildState $state -Outcome $outcome `
            -ArtifactIdentity (NewArtifact) -TerminalReason 'seal_unconfirmed')
        Test-ImageBuildResult -Evidence $evidence | Should -Be $reason
        Test-SealedCandidate -Evidence $evidence | Should -BeFalse
    }

    It 'refuses a failed result carrying an artifact identity' {
        $evidence = AsObject (NewBuildEvidence -BuildState 'seal-unconfirmed' -Outcome 'failed' `
            -ArtifactIdentity (NewArtifact) -TerminalReason 'seal_failed')
        Test-ImageBuildResult -Evidence $evidence | Should -Not -BeNullOrEmpty
        Test-SealedCandidate -Evidence $evidence | Should -BeFalse
    }
    ...
    It 'refuses a sealed result whose seal phase did not pass' {
        $evidence = AsObject (NewBuildEvidence -ArtifactIdentity (NewArtifact) -Phases @(
            [ordered]@{ name = 'construction'; outcome = 'passed'; reasonCode = $null }
            [ordered]@{ name = 'seal';         outcome = 'failed'; reasonCode = 'seal_failed' }))
        Test-ImageBuildResult -Evidence $evidence | Should -Be 'sealed_with_failed_phase'
    }

    It 'refuses a sealed result with no seal phase at all' {
        $evidence = AsObject (NewBuildEvidence -ArtifactIdentity (NewArtifact) -Phases @(
            [ordered]@{ name = 'construction'; outcome = 'passed'; reasonCode = $null }))
        Test-ImageBuildResult -Evidence $evidence | Should -Be 'sealed_without_a_passed_seal_phase'
    }
...
Describe 'an artifact left by a failed evidence emission' {
    ...
    It 'refuses every non-sealed state as a candidate' -ForEach @(
        ...
        $evidence = AsObject (NewBuildEvidence -BuildState $state -Outcome $outcome `
            -TerminalReason 'seal_unconfirmed')
        Test-SealedCandidate -Evidence $evidence | Should -BeFalse
    }
